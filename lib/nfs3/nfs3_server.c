...
static int nfs3_op_setattr(struct rpc_trans *rt)
{
	...
	int ret = vfs_setattr(inode, &rs, &ap);
	if (ret) {
...
static int nfs3_op_mkdir(struct rpc_trans *rt)
{
	...
	int ret = vfs_mkdir(inode, args->where.name, mode, &ap, &new_inode);
	if (ret) {
		res->status = errno_to_nfs3(ret);
		wcc = &resfail->dir_wcc;
		goto update_wcc;
...
static int nfs3_op_symlink(struct rpc_trans *rt)
{
	...
	int ret = vfs_symlink(inode, args->where.name,
			      args->symlink.symlink_data, &ap, &new_inode);
	if (ret) {
		res->status = errno_to_nfs3(ret);
		wcc = &resfail->dir_wcc;
		goto update_wcc;
...
static int nfs3_op_mknod(struct rpc_trans *rt)
{
	...
	int ret =
		vfs_mknod(inode, args->where.name, mode, rdev, &ap, &new_inode);
	if (ret) {
		res->status = errno_to_nfs3(ret);
		wcc = &resfail->dir_wcc;
		goto update_wcc;
...
static int nfs3_op_remove(struct rpc_trans *rt)
{
	...
	int ret = vfs_remove(inode, args->object.name, &ap);
	res->status = errno_to_nfs3(ret);
...
static int nfs3_op_rmdir(struct rpc_trans *rt)
{
	...
	int ret = vfs_rmdir(inode, args->object.name, &ap);
	res->status = errno_to_nfs3(ret);
...
static int nfs3_op_rename(struct rpc_trans *rt)
{
	...
	int ret = vfs_rename(inode_src, args->from.name, inode_dst,
			     args->to.name, &ap);
	res->status = errno_to_nfs3(ret);
