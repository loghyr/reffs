...
int io_lsnr_setup_udp_ipv4(int port)
{
	int listen_fd;
	struct sockaddr_in address;
	...
	if (listen_fd < 0) {
		LOG("IPv4 UDP socket: %s", strerror(errno));
		return -1;
	}
	...
	if (setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) <
	    0) {
		LOG("IPv4 UDP setsockopt: %s", strerror(errno));
		close(listen_fd);
		return -1;
	}
	...
	if (bind(listen_fd, (struct sockaddr *)&address, sizeof(address)) < 0) {
		LOG("IPv4 UDP bind: %s", strerror(errno));
		close(listen_fd);
		return -1;
	}
...
int io_lsnr_setup_udp_ipv6(int port)
{
	int listen_fd;
	struct sockaddr_in6 address;
	...
	if (listen_fd < 0) {
		LOG("IPv6 UDP socket: %s", strerror(errno));
		return -1;
	}
	...
	if (setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) <
	    0) {
		LOG("IPv6 UDP setsockopt: %s", strerror(errno));
		close(listen_fd);
		return -1;
	}
	...
	if (setsockopt(listen_fd, IPPROTO_IPV6, IPV6_V6ONLY, &ipv6only,
		       sizeof(ipv6only)) < 0) {
		LOG("IPv6 UDP setsockopt (v6only): %s", strerror(errno));
		close(listen_fd);
		return -1;
	}
	...
	if (bind(listen_fd, (struct sockaddr *)&address, sizeof(address)) < 0) {
		LOG("IPv6 UDP bind: %s", strerror(errno));
		close(listen_fd);
		return -1;
	}
