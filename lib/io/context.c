...
struct io_context_snapshot *io_context_probe_snapshot(int fd, enum op_type op,
						      ...
						      int *out_count)
{
	struct io_context_snapshot *arr = NULL;
	int capacity = 0;
	int matched = 0;
	...
	for (unsigned int i = 0; i < CONTEXT_HASH_SIZE; i++) {
		for (struct io_context *ic = context_hash[i]; ic != NULL;
			...
			arr[matched].op_type = ic->ic_op_type;
			arr[matched].fd = ic->ic_fd;
			arr[matched].id = ic->ic_id;
			arr[matched].xid = ic->ic_xid;
			arr[matched].buffer_len = ic->ic_buffer_len;
			arr[matched].position = ic->ic_position;
			arr[matched].expected_len = ic->ic_expected_len;
			arr[matched].state = ic->ic_state;
			arr[matched].count = ic->ic_count;
			arr[matched].action_time = ic->ic_action_time;
			arr[matched].ci = ic->ic_ci;
			matched++;
		}
	}
