...
_utf8_val_or_empty(const utf8string *u)
{
	return (u->utf8string_val != NULL) ? u->utf8string_val : "";
}

static inline const char *
_cstr_safe(const char *s)
{
	return (s != NULL) ? s : "";
}
...
int
utf8string_alloc(utf8string *u, unsigned int len)
{
	char *buf;
...
int
utf8string_from_wire(utf8string *dst, const utf8string *wire)
{
	int rc;
 ...
 */
static inline int
_utf8_sequence_len(unsigned char b)
{
	if (b < 0x80)  return 1;   /* 0xxxxxxx — ASCII */
	if (b < 0xC0)  return 0;   /* 10xxxxxx — continuation byte, not a leader */
...
int
utf8string_validate(const utf8string *u)
{
...
int
utf8string_from_wire_validated(utf8string *dst, const utf8string *wire)
{
	int rc;
...
bool
utf8string_is_null(const utf8string *u)
{
...
int
utf8string_copy(utf8string *dst, const utf8string *src)
{
	int rc;
...
void
utf8string_move(utf8string *dst, utf8string *src)
{
	if (dst == src)
		return;
...
static int
_utf8_cmp(const utf8string *a, const utf8string *b,
          int (*cmpfn)(const char *, const char *, size_t))
{
	...
	minlen = (a->utf8string_len < b->utf8string_len)
	       ? a->utf8string_len : b->utf8string_len;

	rc = cmpfn(_utf8_val_or_empty(a), _utf8_val_or_empty(b), minlen);
	if (rc != 0)
		return rc;

	if (a->utf8string_len < b->utf8string_len) return -1;
	if (a->utf8string_len > b->utf8string_len) return  1;
	return 0;
}
...
int
cstr_to_utf8string(utf8string *dst, const char *cstr)
{
	size_t len;
	int    rc;
...
const char *
utf8string_to_cstr(const utf8string *u)
{
	if (u == NULL)
		return NULL;
...
static int
_utf8_cmp_cstr(const utf8string *u, const char *cstr,
               int (*cmpfn)(const char *, const char *, size_t))
{
	const char  *uval;
	size_t       clen;
	...
	if (ulen < (unsigned int)clen) return -1;
	if (ulen > (unsigned int)clen) return  1;
	return 0;
}
 ...
 */
static int
_id_to_utf8string(utf8string *dst, uintmax_t id)
{
	/*
...
int
utf8string_from_uid(utf8string *dst, uid_t uid)
{
	return _id_to_utf8string(dst, (uintmax_t)uid);
}

int
utf8string_from_gid(utf8string *dst, gid_t gid)
{
 ...
 */
static int
_utf8string_to_id(const utf8string *u, uintmax_t max_val, uintmax_t *out)
{
	const char *p;
	uintmax_t   val;
	uintmax_t   digit;
...
int
utf8string_to_uid(const utf8string *u, uid_t *uid_out)
{
	uintmax_t val;
	int       rc;
...
int
utf8string_to_gid(const utf8string *u, gid_t *gid_out)
{
	uintmax_t val;
	int       rc;
...
void
utf8string_print(FILE *f, const utf8string *u)
{
	if (utf8string_is_null(u)) {
		fputs("<null>", f);
		return;
...
void
utf8string_print_repr(FILE *f, const utf8string *u)
{
	const unsigned char *p;
	unsigned int         i;
 ...
 */
static inline bool
_is_forbidden_codepoint(uint32_t cp)
{
	if (cp == 0x0000)                   return true;  /* NUL */
	if (cp <= 0x001F)                   return true;  /* C0 controls */
	if (cp == 0x002F)                   return true;  /* '/' */
	if (cp == 0x007F)                   return true;  /* DEL */
	if (cp >= 0x0080 && cp <= 0x009F)   return true;  /* C1 controls */
	return false;
}

int
utf8string_validate_component(const utf8string *u, unsigned int name_max)
{
	const unsigned char *p;
	const unsigned char *end;
	uint32_t             cp;
	...
	if (u->utf8string_len == 1 && u->utf8string_val[0] == '.')
		return -EBADNAME;
	if (u->utf8string_len == 2 &&
	    u->utf8string_val[0] == '.' && u->utf8string_val[1] == '.')
		return -EBADNAME;
