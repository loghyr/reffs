...
static void parse_server(struct reffs_config *cfg, toml_table_t *srv)
{
	...
	d = toml_int_in(srv, "test_chunk_finalize_delay_count");
	if (d.ok && d.u.i >= 0)
		cfg->test_chunk_finalize_delay_count = (unsigned int)d.u.i;

	d = toml_int_in(srv, "test_chunk_commit_delay_count");
	if (d.ok && d.u.i >= 0)
		cfg->test_chunk_commit_delay_count = (unsigned int)d.u.i;
	...
	d = toml_int_in(srv, "stripe_width");
	if (d.ok && d.u.i > 0)
		cfg->stripe_width = (unsigned int)d.u.i;
}
...
/*
 * Parse a default_coding string into a reffs_coding_spec.
 * Returns 0 on success, -1 on parse error (caller TRACEs and
 * leaves the spec zero-initialised, which the LAYOUTGET dispatch
 * treats as "fall back to PASSTHROUGH with k=ss_layout_width").
 *
 * Accepted forms:
...
static int parse_coding_spec(const char *s, struct reffs_coding_spec *out)
{
	...
	if (n != 3)
		return -1; /* "rs:4" (missing +M) or garbage */

	if (k < 1 || k > LAYOUT_SEG_MAX_FILES)
		return -1;
...
static void parse_one_export(struct reffs_export_config *exp, toml_table_t *tbl)
{
	 ...
	 */
	d = toml_string_in(tbl, "default_coding");
	if (d.ok) {
		if (parse_coding_spec(d.u.s, &exp->default_coding) < 0)
			TRACE("config: invalid default_coding '%s' for export '%s'",
			      d.u.s, exp->path);
		free(d.u.s);
	}
...
int reffs_config_load(struct reffs_config *cfg, const char *path)
{
	...
	if (arr) {
		...
		for (int i = 0; i < nds; i++) {
			...
			d = toml_int_in(ds_tbl, "mount_port");
			if (d.ok && d.u.i > 0 && d.u.i < 65536)
				dsc->mount_port = (uint16_t)d.u.i;
