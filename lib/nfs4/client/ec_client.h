...
/*
 * Timing + count stats from a single ec_repair_encoding invocation;
 * the function itself is declared after `enum ec_encoding_type` below
 ...
 */
/*
 * ec_repair_encoding -- drive the wire-level EC repair end-to-end.
 *
...
int ec_repair_encoding(struct mds_session *ms, const char *path, int k, int m,
		       ...
		       size_t shard_size, size_t file_len,
		       struct ec_repair_stats *out_stats);
