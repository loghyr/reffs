 ...
 * for per-export encoding selection.
 */
...
START_TEST(test_segment_runway_target_drives_k_m)
{
	...
	int rc = default_coding_resolve_segment(&rs, 6, &k, &m, &encoding);

	ck_assert_int_eq(rc, 0);
	ck_assert_uint_eq(k, 4);
	ck_assert_uint_eq(m, 2);
	ck_assert_uint_eq(encoding, FFV2_ENCODING_RS_VANDERMONDE);
}
END_TEST

/*
 * Mojette-sys(8,2), runway popped 10 files.  ls_k=8, ls_m=2,
 * encoding=MOJETTE_SYSTEMATIC.  Pins the encoding-type plumbing for
 * a non-RS spec.
 */
START_TEST(test_segment_mojette_sys_8_2_default)
{
	struct reffs_coding_spec moj = {
		.cs_encoding_type = REFFS_ENCODING_MOJETTE_SYSTEMATIC,
		.cs_k = 8,
		.cs_m = 2,
	};
	uint16_t k = 0, m = 0;
	...
	int rc = default_coding_resolve_segment(&moj, 10, &k, &m, &encoding);

	ck_assert_int_eq(rc, 0);
	ck_assert_uint_eq(k, 8);
	ck_assert_uint_eq(m, 2);
	ck_assert_uint_eq(encoding, FFV2_ENCODING_MOJETTE_SYSTEMATIC);
}
END_TEST
...
START_TEST(test_segment_dstore_count_drops_returns_eagain)
{
	...
	int rc = default_coding_resolve_segment(&rs, 8, &k, &m, &encoding);
 ...
 * nfiles, ls_m = 0, encoding = PASSTHROUGH.  This is the path
...
START_TEST(test_segment_unset_falls_back_to_legacy)
{
	...
	int rc = default_coding_resolve_segment(&unset, 6, &k, &m, &encoding);
 ...
 * "passthrough" via TOML so cs_encoding_type is non-zero).  The
...
START_TEST(test_segment_explicit_passthrough_equals_unset)
{
	struct reffs_coding_spec pt = {
		.cs_encoding_type = REFFS_ENCODING_PASSTHROUGH,
		.cs_k = 0,
		.cs_m = 0,
	};
	uint16_t k = 0, m = 0;
	...
	/* Explicit PASSTHROUGH carries a non-zero encoding_type, so it
	 * is NOT the all-zero "unset" sentinel even though m == 0. */
	ck_assert(!reffs_coding_spec_is_unset(&pt));

	int rc = default_coding_resolve_segment(&pt, 4, &k, &m, &encoding);
