...
`deploy/sanity/run-ps-demo-encodings.sh` documents the gap directly:
...
The encoding itself is now pinned at 24 KiB by
`lib/ec/tests/mojette_encoding_test.c` (commit `a24c5ba882c5`).  The
...
`ec_write_encoding` / `ec_read_encoding`, default 4096 via thin wrappers
...
1. `lib/nfs4/ps/ec_pipeline.c`
   ...
   - Add `size_t shard_size` parameter to `ec_write_encoding`/`ec_read_encoding`.
...
2. `lib/nfs4/client/ec_client.h`
   - `ec_write_encoding` / `ec_read_encoding` signatures grow `shard_size`.
...
3. `tools/ec_demo.c`
   ...
   - Thread through to `ec_write_encoding` / `ec_read_encoding`.
...
`deploy/sanity/run-ps-demo-encodings.sh`.

**Changes**:

1. `deploy/sanity/run-ps-demo-encodings.sh`
   - Drop the `PAYLOAD_MJ_SIZE = 16K` carve-out.
   - Use the same 96 KiB payload as the other encodings.
...
- **`ec_write_encoding` signature change is API-visible.**  Internal
...
- Variable shard size per encoding instance (e.g. RS at one size,
...
3. `tools/ec_demo --encoding mojette-sys --k 4 --m 2 --shard-size 24576`
